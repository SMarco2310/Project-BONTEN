...
class DashboardDataService {
  constructor() {
    this.apiBaseUrl = "/api";
    this.useMockData = false; // Use real database data from PHP
  }
  ...
  async getSummaryMetrics(period = "january") {
    ...
    const response = await fetch(
      `${this.apiBaseUrl}/metrics/summary?period=${period}`
    );
  ...
  async getStatisticsData(period = "january") {
    ...
    const response = await fetch(
      `${this.apiBaseUrl}/metrics/statistics?period=${period}`
    );
  ...
  _getMockStatisticsData(period) {
    const baseData = {
      ...
      sales: [
        32000, 45000, 28000, 52000, 38000, 48000, 55000, 42000, 35000, 58000,
        62000, 48000,
      ],
      returns: [
        8000, 12000, 6000, 10000, 9000, 11000, 13000, 10000, 8000, 14000, 15000,
        11000,
      ],
    };
    ...
    const reviews = [
      {
        ...
        name: "Guy Hawkins",
        comment:
          "Amazing event! The organization was perfect and the atmosphere was incredible.",
        date: "May 29, 2024",
        rating: 5,
        avatar: "#C05F47",
      },
...
class DashboardController {
    ...
    if (searchInput) {
      ...
      searchInput.addEventListener("keypress", (e) => {
        if (e.key === "Enter" && searchInput.value.trim()) {
          window.location.href = `manager_history.php?search=${encodeURIComponent(
  ...
  setupMetricCardClicks() {
    ...
    if (revenueCard) {
      ...
      revenueCard.addEventListener("click", () => {
        window.location.href =
          "../../views/manager_history.php?sort=revenue-desc";
      });
    }

    const ticketsCard = document.querySelector(
      ".metric-card:has(#ticketsSold)"
    );
    ...
    if (ticketsCard) {
      ...
      ticketsCard.addEventListener("click", () => {
        window.location.href =
          "../../views/manager_history.php?sort=tickets-desc";
      });
    }

    const eventsCard = document.querySelector(
      ".metric-card:has(#activeEvents)"
    );
    if (eventsCard) {
    ...
    const ratingCard = document.querySelector(
      ".metric-card.secondary:has(#avgRating)"
    );
    ...
    if (ratingCard) {
      ...
      ratingCard.addEventListener("click", () => {
        window.location.href =
          "../../views/manager_history.php?status=completed";
      });
    }

    const engagementCard = document.querySelector(
      ".metric-card.secondary:has(#engagementRate)"
    );
    if (engagementCard) {
  ...
  async loadSummaryMetrics() {
    try {
      // Use PHP data if available, otherwise fetch from API
      let metrics;
      if (
        ...
        metrics = await this.dataService.getSummaryMetrics(this.currentPeriod);
      }

      const revenueEl = document.getElementById("totalRevenue");
      if (revenueEl) {
        // Format revenue with 2 decimal places
        const formattedRevenue = parseFloat(
          metrics.totalRevenue
        ).toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
        revenueEl.textContent = `${metrics.currency}${formattedRevenue}`;
  ...
  updateChangeIndicator(element, changeValue) {
    ...
    const iconPath = isPositive ? "M7 14l5-5 5 5H7z" : "M7 10l5 5 5-5H7z";
  ...
  async loadStatistics() {
    try {
      ...
      if (
        ...
        const labels = dashboardData.salesData.map((d) => d.month);
        const sales = dashboardData.salesData.map((d) => d.sales);
      ...
      } else if (typeof salesData !== "undefined" && salesData.length > 0) {
        // Fallback to old salesData variable name
        ...
        chartData = await this.dataService.getStatisticsData(
          this.currentPeriod
        );
      }
          ...
          tooltip: {
            ...
            callbacks: {
              label: function (context) {
                ...
                }: GHC ${context.parsed.y.toLocaleString()}`;
              },
            },
          },
        },
  ...
  async loadSecondaryMetrics() {
    try {
      ...
      if (
        typeof dashboardData !== "undefined" &&
        dashboardData.secondaryMetrics
      ) {
        metrics = {
          avgRating: dashboardData.secondaryMetrics.avgRating,
          maxRating: 5,
          engagementRate: dashboardData.secondaryMetrics.engagementRate,
          ratingTrend: "neutral", // Can be calculated if needed
          engagementTrend: "neutral", // Can be calculated if needed
        };
      } else {
        metrics = await this.dataService.getSecondaryMetrics();
      }
  ...
  async loadReviews(searchQuery = "") {
    try {
      // Only load reviews if there's a search query (reviews are already rendered in PHP)
      if (!searchQuery || searchQuery.trim() === "") {
        return; // Keep the PHP-rendered reviews
      }
    ...
    tbody.innerHTML = reviews
      .map(
        (review) => `
            <tr>
                <td>
                    <div class="reviewer-avatar" style="background-color: ${
                      review.avatar
                    }"></div>
                </td>
                <td class="reviewer-name">${this.escapeHtml(review.name)}</td>
                <td class="review-comment" title="${this.escapeHtml(
                  review.comment
                )}">${this.escapeHtml(review.comment)}</td>
